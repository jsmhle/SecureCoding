...
from video_analyzer import is_video_or_gif, extract_frames, analyze_frames_and_summarize
...
def ask_gemini_about_image(image_bytes):
    try:
        ...
        prompt = (
            "너는 딥페이크 이미지 탐지 전문가 AI다. 아래 이미지를 보고 '딥페이크인지 아닌지'를 판단해라.\n"
...
def index():
    if request.method == 'POST':
        ...
        try:
            if is_video_or_gif(file.filename):
                frames = extract_frames(file)
                if not frames:
                    flash("동영상/GIF에서 프레임을 추출할 수 없습니다.")
                    return render_template('index.html')
                
                result, summary = analyze_frames_and_summarize(frames, models, transforms_dict, device, run_model_inference_tta)
                
                first_frame_bytes = io.BytesIO()
                frames[0].save(first_frame_bytes, format='JPEG')
                img_data_for_html = base64.b64encode(first_frame_bytes.getvalue()).decode('utf-8')
                
                return render_template('index.html',
                                       result=result,
                                       confidence=summary, # 신뢰도 대신 요약 정보를 표시
                                       img_data=img_data_for_html,
                                       model_display_name="Video/GIF Analysis",
                                       selected_model='video_analysis',
                                       gemini_opinion="동영상/GIF는 Gemini 분석을 지원하지 않습니다.")
            else:
                ...
                model_names = {
                    'b1': 'EfficientNet-B1', 'v2s': 'EfficientNet-V2S', 
